...
def separate_data(data, split, calculate_input, calculate_output, lookback, lookforward, bootstrap):
	data = data.to_records()
	split_data = dict()
	input_parameters = np.empty((0,0))
	output_parameters = np.empty((0,0))
	output_timelimits = np.empty((0,2),dtype=int) # For each output stores starting and ending indexes of candles involved.
	...
	for i in range(lookback, len(data) - lookforward - 1):
		...
		if bootstrap > 0: # If bootstrapping is required...
			if isinstance(out,tuple): # Tuple data type assumes an output having it's ending candle index at the end of the tuple. 
				ending_candle_index = out[-1]+i # The ending candle (the starting one is "i")
				output_timelimits = np.append( output_timelimits, [[i-lookback, ending_candle_index]], axis=0 ) # Appending time span for a current output
				out = np.array([out[:-1]]) # The tuple without ending candle index serves as output
			else: # Time span used to calculate output always equals the "lookforward"  
				output_timelimits = np.append( output_timelimits, [[i-lookback, i+lookforward]], axis=0 ) # Appending time span for a current output
				out = np.array([out]) 
	...
	input_parameters = np.reshape(input_parameters, (input_overall_len // inp_len, inp_len))
	output_overall_len = len(output_parameters)
	output_parameters = np.reshape(output_parameters, (output_overall_len // out_len, out_len))
	...
	# SH START
	if np.shape(output_timelimits)[0] > 0 and bootstrap > 0: # If time limits for output has been initialized - bootstrapping...
		
		import random
		from datetime import datetime
		random.seed(datetime.now())

		samples_num = np.shape( output_timelimits )[0] # The total number of samples
		samples_drawn = np.zeros(samples_num, dtype=bool) # To remember samples have been drawn and those that haven't
		indexes = np.arange(0,samples_num) # An array of indexes to draw from
		probabilities = [ 1.0/samples_num for i in range(samples_num) ] # Assigning each sample the same probability to be drawn.
		...
		sample_indexes_drawn = np.full( samples_to_draw_num, -1 ) # To store indexes of samples drawn.
		...
		for samples_to_draw_counter in range(samples_to_draw_num):
			...
			if bootstrap == 2:
				uniquenesses = np.ones(samples_num) # An array to store uniqueness of each sample output
				for samples_counter in range(samples_num): # For each sample...
					starting_candle_index = output_timelimits[samples_counter][0]
					ending_candle_index = output_timelimits[samples_counter][1]
					
					overlaps_sum = 0.0
					overlaps_num = 0
					for t in range( starting_candle_index, ending_candle_index+1): # For each t in the output interval of a current sample...
						overlaps = 0
						for i in range(samples_num): # Calculating overlaps with sample 'i' at time 't'
						...
						overlaps_num += 1
					uniquenesses[samples_counter] = overlaps_sum / overlaps_num # Average uniqueness of output along all it's "t"s
				sum_of_uniquenesses = np.sum(uniquenesses) # The overall 
				for samples_counter in range(samples_num):
					probabilities[samples_counter] = uniquenesses[samples_counter] / sum_of_uniquenesses # Updating probability for each sample output
			# End of updating probabilities

		bootstrapped_input = np.empty((0,inp_len)) # New bootstrapped arrays for inputs 
		bootstrapped_output = np.empty((0,out_len)) # and outputs
		for samples_to_draw_counter in range(samples_to_draw_num): # Initializing... 
			index = sample_indexes_drawn[samples_to_draw_counter]
			bootstrapped_input = np.append( bootstrapped_input, [input_parameters[index]], axis=0 )
			bootstrapped_output = np.append( bootstrapped_output, [output_parameters[index]], axis=0 )
