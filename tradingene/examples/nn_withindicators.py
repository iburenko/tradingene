...
INDICATORS = {'sma2':('sma', 2, 'close'), 'sma4':('sma', 4), 'sma8':('sma', 8), 'sma16':('sma', 16) } # Indicators to feed the NN with.
...
def prepare_model():
    ...
    m.add(Dense(units=NUM_FEATURES*2, activation='tanh', input_dim=NUM_FEATURES, kernel_initializer=he_uniform(1)))
    ...
    m.fit(data['train_input'], one_hot_train_outputs, epochs=NUM_EPOCHS) # Training the model
    return m
# end of load_data
...
def onBar(instrument):
    ...
    inp = calculate_input( rates ) # Passing rates and indicators values into "calculate_input"
