...
Instead of trying to predict classes a trading network may learn to predict a return which is even more valuable. If so you code your script slightly differently.
First of all the ```calculate_output()``` function should returns not a class but a value of return in either form, like this:
...
Next ```categorical_crossentropy``` loss function would be useless and must be substituted with another one, e.g. the ```mean_squared_error```:

```python
    model.compile(loss='mean_squared_error', optimizer='sgd', metrics=['accuracy'])
```

As well you do not need one-hot coding anymore.
